 ...
 * @author Tim Pierson, Dartmouth CS 10, Fall 2018
 * @author Sanjana Goli and Rohith Mandavilli - constructed BFSTree method
...
public class GraphTraversal<V,E> {
	 ...
	 * @param start -- starting vertex
	 */
	public void BFS(Graph<V,E> G, V start) {
		backTrack = new HashMap<V,V>(); //initialize backTrack
	...
	public Graph<V,E> BFSTree(Graph<V,E> G, V start){
		BFS(G, start);
		
		Graph<V,E> tree = new AdjacencyMapGraph<V,E>();
        //inserts all the vertices from the adjacency map graph into the tree
        for(V vertex: G.vertices()){
        	//checks if the vertex is connected to the root****
        	if(findPath(start, vertex).size()>0) tree.insertVertex(vertex);
        }
		for(V vertex: backTrack.keySet()){
			if(!vertex.equals(start))
				tree.insertDirected(vertex, backTrack.get(vertex), G.getEdge(vertex, backTrack.get(vertex)));
		}
